...
| `stop-guard` | `Stop` | a `running` plan with open `- [ ]`; no `.claude/plan-pause`; no `NEED_HUMAN` in the last message; continuations this session below `CC_STOP_GUARD_CAP`=300 | **refuses to let the turn end**: "the plan has N open tasks, next are …, continue; `[x]` only after the check passes; `[!] BLOCKED` only for irreversible actions, missing credentials or a missing dependency". Desktop notification on every exit: `NEED_HUMAN` ("Claude needs you"), zero open ("Plan finished"), ceiling reached ("Stop-guard cap reached") |
